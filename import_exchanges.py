...
if incremental == 0:
    ...
    cur.execute(
        "create table if not exists Exchanges (MIC VARCHAR, Operating MIC VARCHAR, Name VARCHAR, Corp Exchange VARCHAR, Exch Code VARCHAR, Country VARCHAR);")
...
with open(file, newline='') as csvfile:
    ...
    for row in reader:
        if i > 0:
            cur.execute("insert into Exchanges values (:v0,:v1,:v2,:v3,:v4,:v5)",
                        {'v0': row[0], 'v1': row[1], 'v2': row[2], 'v3': row[3], 'v4': row[4], 'v5': row[7]})
